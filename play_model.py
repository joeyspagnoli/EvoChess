...
def draw_promotion_overlay(screen, board_size, piece_color):
    ...
    piece_size = 70
    spacing = 90
    start_x = dialog_x + (dialog_width - (len(pieces) * spacing - (spacing - piece_size))) // 2
    piece_y = dialog_y + 50
    ...
    for i, (piece, name) in enumerate(zip(pieces, piece_names)):
        ...
        label_rect = label_text.get_rect(center=(x + piece_size//2, piece_y + piece_size + 15))
            ...
            if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
                ...
                if selected_square is None:
                    ...
                    if is_promotion:
                        ...
                        temp_board.set_piece_at(destination, piece)  # Move pawn to destination
                        temp_board.remove_piece_at(selected_square)  # Remove pawn from origin
                        
                        # Get promotion choice using pygame UI with pawn shown on destination
                        promotion_choice = get_promotion_choice(screen, board_size, piece.color, temp_board)
